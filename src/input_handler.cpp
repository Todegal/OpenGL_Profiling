...
void InputHandler::pollAxes()
{
        ...
        for (const auto& map : axesMappings)
        {
                ...
                {
                        if (glfwGetKey(windowPtr, k) != GLFW_RELEASE) { mod += map.scale; }
                }
                ...
                for (const auto x : map.gamepadAxis)
                {
                        if (glfwJoystickPresent(GLFW_JOYSTICK_1) && glfwJoystickIsGamepad(GLFW_JOYSTICK_1))
                        {
                                ...
                                {
                                        if (std::abs(state.axes[x]) > 0.01f) { mod += map.scale * state.axes[x]; }
                                }
                        }
                }
...
void InputHandler::pollActions()
{
        ...
        for (auto& [name, map] : actions)
        {
                ...
                for (const auto k : map.keys)
                {
                        if (glfwGetKey(windowPtr, k) != GLFW_RELEASE) { value = true; }
                }

                for (const auto m : map.mouseButtons)
                {
                        if (glfwGetMouseButton(windowPtr, m) != GLFW_RELEASE) { value = true; }
                }
                ...
                for (const auto g : map.gamepadButtons)
                {
                        if (glfwJoystickPresent(GLFW_JOYSTICK_1) && glfwJoystickIsGamepad(GLFW_JOYSTICK_1))
                        {
                                ...
                                if (glfwGetGamepadState(GLFW_JOYSTICK_1, &state))
                                {
                                        if (state.buttons[g] != GLFW_RELEASE) { value = true; }
                                }
                        }
                }
...
void InputHandler::pollToggles()
{
        ...
        for (auto& [name, map] : toggles)
        {
                ...
                for (const auto k : map.keys)
                {
                        ...
                        else if (glfwGetKey(windowPtr, k) == GLFW_RELEASE) { map.previous = false; }
                }
                ...
                for (const auto m : map.mouseButtons)
                {
                        ...
                        else if (glfwGetMouseButton(windowPtr, m) == GLFW_RELEASE) { map.previous = false; }
                }

                if (mod) map.state = !map.state;
        }
}
                              ...
                              const std::vector<int>& gamepadAxis, float scale)
{
        ...
        if (axesValues.find(name) == axesValues.end()) { axesValues[name] = 0.0f; }
}

float InputHandler::getAxis(const std::string& name)
{
        if (axesValues.find(name) != axesValues.end()) return axesValues[name];
...
{
        if (actions.find(name) != actions.end()) return actions[name].state;

        return false;
}

void InputHandler::defineToggle(const std::string& name, const std::vector<int>& keys,
                                const std::vector<int>& mouseButtons, bool initialValue)
{
        ToggleMapping map;
        map.keys = keys;
        map.mouseButtons = mouseButtons;
...
{
        if (toggles.find(name) != toggles.end()) return toggles[name].state;
