...
void ImGuiProfileVisualiser::updateInitEvents(const std::vector<Profiler::ProfileEvent>& initEvents)
{
    ...
    const std::vector<Profiler::ProfileEvent>& events)
{
        ...
        for (const auto& event : events)
        {
                ...
                else { stack.back()->children.push_back(node); }
...
void ImGuiProfileVisualiser::renderTreeNodeRecursive(std::shared_ptr<EventNode> node)
{
        ...
        else { ImGui::TreeNodeEx(label.c_str(), ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_NoTreePushOnOpen); }
...
void ImGuiProfileVisualiser::renderTimeline()
{
	...
	canvasSize.y -= 40;
        ...
        {
                float x = canvasPos.x + i * tickSpacing;
                float timeRatio = static_cast<float>(i) / numTicks;
                auto tickTime = std::chrono::nanoseconds(static_cast<long long>(totalFrameTime.count() * timeRatio));
...
ImVec2 ImGuiProfileVisualiser::calculateNodePosition(std::shared_ptr<EventNode> node, const ImVec2& canvasSize) const
{
        ...
        float y = node->depth * DEPTH_HEIGHT + TIMELINE_PADDING;
        return ImVec2(x, y);
}
